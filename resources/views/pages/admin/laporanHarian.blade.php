...
@section('content')
    <div class="max-w-xl mx-auto bg-white dark:bg-gray-900 border border-gray-200 rounded-lg shadow-lg p-6 mt-8">
        ...
        <table class="w-full mb-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <tbody>
                ...
                @foreach ($pendapatanHarian as $item)
                    @foreach ($item->detailTransaksi as $detail)
                        <tr>
                            <td class="px-4 py-2 text-left text-gray-900 dark:text-white">{{ $loop->iteration }}</td>
                            <td class="px-4 py-2 text-left text-gray-900 dark:text-white">{{ $detail->menu->Menu }}
                            </td>
                            <td class="px-4 py-2 text-left text-gray-900 dark:text-white">{{ $detail->qty }}</td>
                            <td class="px-4 py-2 text-left text-gray-900 dark:text-white">{{ $detail->meja }}</td>
            ...
            <tfoot>
                <tr>
                    <td colspan="4" class="px-4 py-2 text-left font-semibold text-gray-900 dark:text-white">Total</td>
                    <td class="px-4 py-2 font-semibold text-center text-gray-900 dark:text-white">{{ $totalAmount }}</td>
                </tr>
            </tfoot>
        </table>
    ...
    {{-- @foreach ($pendapatanHarian as $key)
            <p>tes</p>
        @endforeach --}}
